						...
						<div class="col-lg-12">
								<div class="row">
									<div class="form-group">
										<label>分组编号:</label> <input class="form-control" disabled="disabled" style="color:red" value="系统自动生成"/>
									</div>
	
									<div class="form-group">
										<label>所属赛事:</label>
										<select name="matchId" id="matchId"  class="form-control">
							    			<c:forEach items="${matchs}" var="item">
												<option value="${item.matchId }">${item.matchName }</option>
											</c:forEach>
										</select>
									</div>
	
									<div class="form-group">
										<label>分组名称:</label> <input class="form-control" id="groupName" name="groupName" value="">
									</div>
	
									<div class="form-group">
										<label>报名费￥:</label> <input class="form-control" id="fee" name="fee" onkeyup="return ValidateNumber($(this),value)">
									</div>
	
									<div class="form-group">
										<label>外籍报名费$:</label> <input class="form-control" id="fee2" name="fee2" onkeyup="return ValidateNumber($(this),value)">
									</div>
								</div>
						</div>
					</form>
...
function submitForm() {	
	...
	var options=$("#matchId option:selected");
	var matchId = options.val();
	$.ajax({
		type : "post",
		data : {
			'matchId' : matchId,
		...
		url : '${marathon}/admin/system/matchGroupMng/saveMatchGroup.shtml',
		dataType:'json',
