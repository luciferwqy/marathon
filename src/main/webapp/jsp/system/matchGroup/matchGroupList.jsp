					...
					<div class="panel panel-default">
						...
						<div class="panel-heading">
						  <h3 class="panel-title panel-h3">
						   <i class="fa fa-table fa-fw"></i><span>组别列表</span>
						  </h3>
						</div>  
						<div class="panel-body table-responsive">
							<table id="matchGroupTable" class="table table-bordered table-hover dataFilter">
								<thead>
									<tr>
										<th>操作</th>
										<th>赛事名称</th>
	      ...
	      <div class="modal-content">
	         <div class="modal-header">
	            <button type="button" class="close"  aria-hidden="true" onclick="closeAddModal()">&times;</button>
	         	<h4 class="modal-title" id="modelTitle">组别新增</h4>
	         </div>
	         <div class="modal-body"  >
	         <iframe id="addIFrame" width="100%" height="100%" frameborder="0" scrolling="auto"></iframe>
	         </div>
	         <div class="modal-footer">
	         	<button type="button" class="btn btn-primary" id="saveMatchGroup">新增</button>
	      ...
	      <div class="modal-content">
	         <div class="modal-header">
	            <button type="button" class="close"  aria-hidden="true" onclick="closeEditModal()">&times;</button>
	         	<h4 class="modal-title" id="modelTitle">组别修改</h4>
	         </div>
	         <div class="modal-body"  >
	         <iframe id="editIFrame" width="100%" height="100%" frameborder="0" scrolling="auto"></iframe>
	         </div>
	         <div class="modal-footer">
	         	<button type="button" class="btn btn-primary" id="updateMatchGroup">修改</button>
...
<script type="text/javascript">
	...
	options = {
			queryForm : $(".query"),
			url :  "${marathon}/admin/system/matchGroupMng/dataList.shtml",
			numPerPage:"10",
			currentPage:"",
			index:"1",
	 ...
	 */
	 function rebuildTable(data){	
		...
		for (var i = 0; i < list.length; i++) {
			str += "<tr><td>";
			if("${privilege==2}"=="true"){
				str += "<button type='button' class='btn btn-warning' onclick='showEditModal("+list[i].groupId+")'>修改</button>" ;
			}
			if("${privilege==3}"=="true"){
				str += "<button type='button' class='btn btn-warning' onclick='showEditModal("+list[i].groupId+")'>修改</button>&nbsp;&nbsp;<button type='button' class='btn btn-danger' onclick='delGroup("+list[i].groupId+")'>删除</button>" ;
			}
			str += "</td><td>" + list[i].matchName;
			str += "</td><td>" + list[i].groupName;
			str += "</td><td>" + list[i].fee;
			str += "</td><td>" + list[i].fee2;
			str += "</td><td>" + list[i].createTime;
			str += "</td><td>" + list[i].updateTime;
			str += "</td><td>" + list[i].opt;
			str += "</td></tr>";
		}
		$("#matchGroupTable tbody").html(str);	
	}
	...
	$(function(){
		 ...
		 $("#saveMatchGroup").click(function(){
			  $("#addIFrame")[0].contentWindow.submitForm();
		 });
		 
		  $("#updateMatchGroup").click(function(){
	 ...
	 function showEditModal(value){
        var frameSrc = '${marathon}/admin/system/matchGroupMng/editGroup.shtml?groupId='+value;
	...
	function showAddFunc(){
        var frameSrc = '${marathon}/admin/system/matchGroupMng/addMatchGroup.shtml';
	...
	function delGroup(value){
		$.zzComfirm.alertConfirm("删除将导致数据消失且不可逆转，确定要删除吗？",$("body"),function(){
			$.ajax({
				type:'post',
				url:'${marathon}/admin/system/matchGroupMng/delGroup.shtml',
				data:{"groupId":value},
				dataType:'json',
